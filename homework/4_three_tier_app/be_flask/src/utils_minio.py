...
def get_user_usage_bytes(username: str, minio_client: S3Client) -> int:
    ...
    Returns:
        Total bytes used
    """
    return minio_client.get_user_usage(username)
...
def get_user_files_list(
    ...
    """
    return minio_client.list_files(username, prefix=subpath, recursive=False)
...
def move_to_bin(
    ...
    # Move the file or directory in MinIO
    if is_directory:
        success = minio_client.move_directory_to_bin(username, item_path, bin_path)
    else:
        success = minio_client.move_file(username, item_path, bin_path)
...
def restore_from_bin(
    ...
    if bin_item.item_type == 'directory':
        success = minio_client.restore_directory_from_bin(
            username, 
            bin_item.bin_path, 
        ...
        success = minio_client.move_file(username, bin_item.bin_path, bin_item.original_path)
...
def permanently_delete_from_bin(
    ...
    if bin_item.item_type == 'directory':
        success = minio_client.delete_directory(username, bin_item.bin_path)
    else:
...
def cleanup_expired_bin_items(minio_client: S3Client) -> int:
    ...
    for item in expired_items:
        try:
            # Delete from MinIO
            minio_client.delete_file(item.username, item.bin_path)
...
def get_directory_size(username: str, dir_path: str, minio_client: S3Client) -> int:
    ...
    files = minio_client.list_files(username, prefix=prefix, recursive=True)
